...
# with the first argument.  _generateproof_api #############
...
# replaces the current idea of encrypting a password and ###################
# having a username stored.  This idea involves storing the username #######
...
# acts as a password as well, and the salt to the proof is not stored. #####
# The other benefit is that it also increases security by not exposing a ###
# username due to database dumping, just a usesless proofcode is shown. ####
############################################################################
...
############################################################################
# In version 0.7.4, the $returnusername argument was repaired to truely ####
# return just the username.  To prevent a slight loophole during a #########
# registration machnism where, if an individual guesses the username and ###
# password in the registration form, (s)he could use that knowlege to ######
# succesfully login, the username is NOT encrypted with the password. ######
# This is contrary to the real Ragnarok algorithm where matching is key. ###
# Note: the only reason the username is not using cascading encryption is ##
# to make sure that this security whole is closed.  For every other ########
# instance of the Ragnarok algorithm, this manuver is not necessary and ####
# foolish as it lends itself to weakening the inherent strength of #########
# the algorithm. ###########################################################
############################################################################
...
sub _generateupass {
    ...
    $AboutKey = $username;
    return _generatekey($AboutKey) if $returnusername;
    $AboutCode = $password;

    # The Key is autogenerated in the _generatecode algorithm.
    $Code = _generatecode($AboutCode);
    return $Key if ($returnusername);
    return _generateproof_api($Key, $Code);
} ## end sub _generateupass
